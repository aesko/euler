...
greatest_prime_factor <- function(x) {
  if(num %% x == 0){
    if(is.prime(num/x)){return(num/x)
      } else greatest_prime_factor(x+2)
  } else greatest_prime_factor(x+2)
}
greatest_prime_factor(3)



div <- 2:100000 %>% map_lgl(~ num %% . == 0) %>% which() +1 %>% as.integer()
x <- num/div %>% map_lgl(is.prime)
